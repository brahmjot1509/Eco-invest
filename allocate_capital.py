...
def allocate_capital(E_scr, S_scr, G_scr, A, sectors=None, stocks=None):
    ...
    prices_path = os.path.join("static", "data", "stock_prices.csv")  
    prices = pd.read_csv(prices_path, index_col='date', parse_dates=True)  

    #Read in esg score data:
    esg_path = os.path.join("static", "data", "stock_data.csv") 
...
def get_portfolio(E_scr, S_scr, G_scr, A, sectors=None, stocks=None):
    ...
    allocations[['Allocations','Sectors']].to_csv(os.path.join("static", "data", "portfolio.csv"))
    ...
    fig_stocks.update_traces(textposition='inside', textfont_size=15,marker=dict(line=dict(color='#000000', width=1)))
    ...
    fig_sectors.update_traces(textposition='inside', textfont_size=15,marker=dict(line=dict(color='#000000', width=1)))
